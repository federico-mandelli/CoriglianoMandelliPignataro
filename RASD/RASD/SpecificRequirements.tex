...
\paragraph{\ac{eMSP}}
The \ac{eMSP} should be accessible through an application installed on the mobile device of the user.
...
\paragraph{\ac{eMSP}}
The user, in order to interact with the \ac{eMSP}, must have a device that is provided with a \ac{GPS} and internet connection. Thanks to this, the user can search for close charging stations, see if those are available and can book a charge or cancel a previous one.
...
\todo[inline]{"The Charging sockets must have an input method for inserting the pin so that the user can validate the charge." added to assumptions/constraints}

\subsubsection{Software interfaces}
\paragraph{\ac{eMSP}}
In order to provide all the functionalities, the \ac{eMSP} should provide the following software inferfaces:
\begin{itemize}
    ...
    \item Confirm the registration of a given user thanks to an ID;
    \item Login to the \ac{eMSP} providing a correct email and password pair;
    \item Retrieving data about charging stations, such as the nearest (providing location), the cheapest or a list of K charging stations
    \item Book a charge providing charging station and timeslot information; \todo[inline]{and type of charge?}
    \item Cancel an already booked charge;
    \item Paying a charge with the already setted payment method;
    \item A \ac{CPO} can add a \ac{CPMS} providing a link to its interface;
\end{itemize}
...
\begin{itemize}
    ...
    \item Enable a charge on a charging station, passing the ID (a PIN in order to authorize the charge once the user gets in the station), the station and the timeslot;
...
\begin{enumerate}[label=\textbf{R\arabic*}]
    ...
    \item The \ac{eMSP} shall reserve a socket in the right charging station with for a user who booked a charge through the application;\label{req:eMSP-reserve-socket}
    ...
    \item the \ac{eMSP} shall allow to add to an already registered \ac{CPO} a \ac{CPMS}, providing \ac{API} reference to the \ac{CPMS};\label{req:eMSP-add-cpms}
    ...
    \item The \ac{eMSP} shall allow the \ac{CPO} to set special offers;\label{req:eMSP-set-special-offers}\todo[inline]{Check in the UML and sequence if this is possible}
    \item The \ac{CPMS} shall be reachable by \acp{eMSP} in order to perform/cancel a booking, or query the system;\label{req:CPMS-reachable}
